...
export function classifyHermesRunOutcome(input: {
  ...
  if (input.error && /evidence is invalid|parameter repair rejected|research turn (?:must contain exactly one action|contained multiple actions)|no[_ ]progress|repeated failed (fetches|searches)/i.test(input.error)) return 'NO_PROGRESS'
...
async function executeClaim(task: any, leaseId: string | null = null, oneShot = false, existingRunId?: string, acknowledgeOnly = false): Promise<{ ok: boolean; accepted?: boolean; message: string; run_id?: string }> {
  ...
  try {
    ...
    const baseSystemMessage = `Mission Control background COO execution. You are operating only on the server-authorized tenant ${task.tenant_id}, project ${task.project_id}, task ${task.id}. No shell, PTY, process spawn, credentials, filesystem mutation, financial action, deployment, or architecture change is available. ${research ? 'This is an evidence-first research task. Complete only the current server-selected objective. One research turn = exactly ONE bounded action: emit one action object only, never an array, never multiple tool/action objects, and stop immediately after its closing </mc_action> tag. Do not emit a plan or prose. If a fetch fails, choose only the next single action; do not combine diagnosis, fetch, and evidence. For retailer research, use one FETCH_PUBLIC_JSON_API action for a fallback endpoint when needed, then use a later turn for SAVE_RESEARCH_EVIDENCE only after an eligible current-run source exists. Never combine fetch and SAVE_RESEARCH_EVIDENCE in one turn. Preserve exact URLs, dates, and evidence classifications; do not invent access, legal, licensing, or commercial conclusions.' : ''} You may emit only these bounded actions: SAVE_WORKING_MEMORY, CREATE_TASK (must be assigned to yourself), UPDATE_TASK_RESULT (current task only), REQUEST_CEO_APPROVAL, SEARCH_WEB, FETCH_PUBLIC_URL, FETCH_PUBLIC_JSON_API, SAVE_RESEARCH_EVIDENCE. Do not create follow-up tasks unless strictly required by the task and never create more than one. Project context: ${JSON.stringify(context)}\nTask: ${JSON.stringify({ id: task.id, title: task.title, description: task.description, priority: task.priority })}`
